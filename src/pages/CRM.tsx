...
import {
  FanFaveLogo, StatusPill, CanalPill,
  Btn, StatCard, Toast, Modal,
  InputField, SelectField, StatusSelector,
} from '../components/ui';
import {
  getLeads, createLead, updateLead, deleteLead, getStats,
  getNotifs, saveNotifs, pushNotif, fmtDate, timeAgo,
} from '../lib/storage';
import {
  COLORS, STATUS_ORDER, STATUS_LABEL, STATUS_COLOR,
  NEXT_STATUS, CANAL_LABEL, SEGMENTS, CHART_COLORS,
} from '../lib/constants';
...
function Reports({ leads }: { leads: Lead[] }) {
  ...
  const canalCounts = leads.reduce<Record<string, number>>((acc, l) => {
    const c = l.origem ?? 'manual'; acc[c] = (acc[c] ?? 0) + 1; return acc;
  }, {});
  const melhorCanal = Object.keys(canalCounts).reduce(
    (a, b) => (canalCounts[a] ?? 0) > (canalCounts[b] ?? 0) ? a : b, '—'
  );
  ...
  const tipoData = Object.entries(
    leads.reduce<Record<string, number>>((a, l) => { a[l.tipo] = (a[l.tipo] ?? 0) + 1; return a; }, {})
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ background: COLORS.white, border: `1px solid ${COLORS.grayBorder}`, borderRadius: 12, padding: 18 }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>Evolução — últimos 14 dias</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.grayBorder}`, borderRadius: 12, padding: 18 }}>
          ...
          <ResponsiveContainer width="100%" height={140}>
            <PieChart>
              <Pie data={tipoData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={55} innerRadius={30}>
        ...
        <div style={{ background: COLORS.white, border: `1px solid ${COLORS.grayBorder}`, borderRadius: 12, padding: 18 }}>
          ...
          {Object.entries(canalCounts).map(([c, n]) => {
            ...
            const col: Record<string, string> = { instagram: '#E1306C', landing: COLORS.blue, whatsapp: '#25D366', indicacao: COLORS.amber, manual: '#9CA3AF' };
            return (
              <div key={c} style={{ marginBottom: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 3 }}>
                  <span style={{ fontWeight: 500 }}>{CANAL_LABEL[c] ?? c}</span>
                  <span style={{ color: COLORS.grayText }}>{n}</span>
                </div>
                <div style={{ height: 8, background: COLORS.grayBg, borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${pct}%`, background: col[c] ?? '#9CA3AF', borderRadius: 4 }} />
                </div>
              </div>
            );
          })}
          ...
          return (
            <div key={f.name} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <div style={{ flex: 1, height: 28, background: COLORS.grayBg, borderRadius: 6, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${pct}%`, background: f.color, borderRadius: 6, display: 'flex', alignItems: 'center', paddingLeft: 8, minWidth: 24 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: '#fff' }}>{f.value}</span>
                </div>
              </div>
      ...
      {STATUS_ORDER.map((s) => {
        ...
        return (
          <div key={s} style={{ background: COLORS.grayBg, border: `1px solid ${COLORS.grayBorder}`, borderRadius: 12, padding: 10 }}>
            ...
            {items.map((l) => (
              <div key={l.id} onClick={() => onOpen(l)} style={{ background: COLORS.white, border: `1px solid ${COLORS.grayBorder}`, borderRadius: 8, padding: 10, marginBottom: 7, cursor: 'pointer' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 4, marginBottom: 5 }}>
                  ...
                  <span style={{ fontSize: 9, padding: '2px 5px', borderRadius: 6, background: COLORS.grayBg, color: COLORS.grayText, border: `1px solid ${COLORS.grayBorder}`, flexShrink: 0, whiteSpace: 'nowrap' }}>{l.tipo}</span>
                </div>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  ...
                  {NEXT_STATUS[s] && (
                    <button onClick={(e) => { e.stopPropagation(); onAdvance(l.id as number); }} style={{ flex: 1, height: 24, border: `1px solid ${COLORS.blue}`, borderRadius: 6, background: COLORS.blue, fontSize: 10, cursor: 'pointer', color: '#fff', fontWeight: 500 }}>
                      → {STATUS_LABEL[NEXT_STATUS[s]]}
                    </button>
                  )}
                </div>
              </div>
  ...
  return (
    <div style={{ background: COLORS.white, border: `1px solid ${COLORS.grayBorder}`, borderRadius: 12, overflow: 'hidden' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
        <thead>
          <tr style={{ background: COLORS.grayBg, borderBottom: `1px solid ${COLORS.grayBorder}` }}>
            {['Nome', 'Estabelecimento', 'Canal', 'Status', 'Data', ''].map((h, i) => (
              <th key={i} style={{ fontSize: 10, fontWeight: 600, color: COLORS.grayText, textTransform: 'uppercase', letterSpacing: '0.05em', padding: '9px 12px', textAlign: 'left', width: i === 5 ? 50 : undefined }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          ...
          {leads.map((l) => (
            <tr key={l.id} onClick={() => onOpen(l)} style={{ cursor: 'pointer', borderBottom: `1px solid ${COLORS.grayBorder}` }}>
              ...
              <td style={{ padding: '10px 12px' }}>
                <button onClick={(e) => { e.stopPropagation(); onOpen(l); }} style={{ height: 24, padding: '0 8px', border: `1px solid ${COLORS.grayBorder}`, borderRadius: 6, background: COLORS.white, fontSize: 10, cursor: 'pointer', color: COLORS.grayText }}>Abrir</button>
              </td>
            </tr>
          ))}
        </tbody>
...
export default function CRM() {
  ...
  function login() {
    if (loginUser === 'lucas' && loginPass === 'fanfave2026') setAuthed(true);
    else setLoginErr(true);
  }
  function showToast(msg: string) { setToast(msg); }
  function addNotif(msg: string) { pushNotif(msg); setNotifs(getNotifs()); }
  ...
  async function saveLeadDetail() {
    ...
    try {
      ...
      if (editStatus !== detailLead.status) addNotif(`${detailLead.nome} movido para "${STATUS_LABEL[editStatus]}"`);
  ...
  async function advance(id: number) {
    ...
    try {
      ...
      addNotif(`${l.nome} avançou para "${STATUS_LABEL[next]}"`);
      showToast(`Avançado → ${STATUS_LABEL[next]}`);
    } catch {
      showToast('Erro ao avançar lead');
    }
  }
  ...
  async function addLead() {
    ...
    try {
      ...
      addNotif(`Novo lead: ${nome} (${estab}) via ${CANAL_LABEL[canal]}`);
      setAddOpen(false);
      setAddForm(EMPTY_ADD);
      showToast('Lead adicionado!');
    } catch (err: unknown) {
      showToast(err instanceof Error ? err.message : 'Erro ao adicionar lead');
    }
  }

  function exportCSV() {
    const hdr = ['Nome','WhatsApp','Email','Estabelecimento','Tipo','Cidade','Canal','Status','Data'];
    const rows = leads.map((l) => [l.nome,l.whatsapp,l.email??'',l.estabelecimento,l.tipo,l.cidade??'',CANAL_LABEL[l.origem]??l.origem,STATUS_LABEL[l.status]??l.status,fmtDate(l.data)].map((v)=>`"${(v??'').replace(/"/g,'""')}"`).join(','));
    const a = document.createElement('a');
    a.href = 'data:text/csv;charset=utf-8,\uFEFF' + encodeURIComponent(hdr.join(',') + '\n' + rows.join('\n'));
    a.download = `fanfave_leads_${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
  }
  ...
  if (!authed) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: COLORS.blue }}>
      <div style={{ background: COLORS.white, borderRadius: 16, padding: 36, width: 320, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        ...
        {loginErr && <div style={{ background: '#FEE2E2', color: '#DC2626', fontSize: 12, padding: '9px 12px', borderRadius: 8, marginBottom: 12, border: '1px solid #FECACA' }}>Usuário ou senha incorretos.</div>}
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', color: COLORS.text, background: COLORS.grayBg, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ height: 52, background: COLORS.white, borderBottom: `1px solid ${COLORS.grayBorder}`, display: 'flex', alignItems: 'center', padding: '0 16px', gap: 10, flexShrink: 0, position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <button onClick={refresh} style={{ height: 30, padding: '0 12px', border: `1px solid ${COLORS.grayBorder}`, borderRadius: 7, background: COLORS.white, fontSize: 12, cursor: 'pointer' }}>↻ Atualizar</button>
          ...
          {notifOpen && (
            <div style={{ position: 'absolute', top: 38, right: 0, width: 280, background: COLORS.white, border: `1px solid ${COLORS.grayBorder}`, borderRadius: 12, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', zIndex: 200 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderBottom: `1px solid ${COLORS.grayBorder}` }}>
                ...
                <button onClick={() => setNotifOpen(false)} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: 13, color: COLORS.grayText }}>✕</button>
              </div>
        ...
        <div style={{ width: 190, background: COLORS.white, borderRight: `1px solid ${COLORS.grayBorder}`, padding: '10px 8px', overflowY: 'auto', flexShrink: 0 }}>
          {(['pipeline', 'relatorios'] as const).map((pg) => (
            <div key={pg} onClick={() => setPage(pg)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 7, cursor: 'pointer', fontSize: 13, marginBottom: 1, background: page === pg ? COLORS.blueLight : COLORS.white, color: page === pg ? COLORS.blue : COLORS.grayText, fontWeight: page === pg ? 500 : 400 }}>
              <span style={{ fontSize: 14, width: 16 }}>{pg === 'pipeline' ? '📋' : '📊'}</span>
              {pg === 'pipeline' ? 'Pipeline' : 'Relatórios'}
            </div>
          ))}
          ...
          {([['🔵','Todos',''], ['🆕','Novos','novo'], ['📞','Em contato','contato'], ['📱','Demo','demo'], ['🤝','Negociação','negociacao'], ['✅','Fechados','fechado']] as const).map(([ico, lbl, s]) => (
          ...
          {([['📡','Todos',''], ['📸','Instagram','instagram'], ['🌐','Landing','landing'], ['💬','WhatsApp','whatsapp'], ['⭐','Indicação','indicacao'], ['✏️','Manual','manual']] as const).map(([ico, lbl, c]) => (
          ...
          {page === 'pipeline' && (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
                ...
                <StatCard val={novos} lbl="Novos" delta={novos > 0 ? `${novos} aguardando contato` : undefined} />
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ display: 'flex', border: `1px solid ${COLORS.grayBorder}`, borderRadius: 7, overflow: 'hidden', marginLeft: 'auto' }}>
                  {(['kanban', 'list'] as const).map((v) => (
                    <button key={v} onClick={() => setView(v)} style={{ padding: '5px 12px', fontSize: 12, fontWeight: 500, cursor: 'pointer', border: 'none', background: view === v ? COLORS.blueLight : COLORS.white, color: view === v ? COLORS.blue : COLORS.grayText }}>
              ...
              {view === 'kanban'
                ? <KanbanView leads={filtered} onOpen={openDetail} onAdvance={advance} />
                : <ListView leads={filtered} onOpen={openDetail} />
              }
            </>
          )}
      ...
      <Modal open={!!detailLead} onClose={() => setDetailLead(null)} title={detailLead?.nome ?? ''}
        ...
        {detailLead && (
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {([['Estabelecimento', detailLead.estabelecimento], ['Tipo', detailLead.tipo], ['WhatsApp', detailLead.whatsapp], ['E-mail', detailLead.email], ['Cidade', detailLead.cidade], ['Data', fmtDate(detailLead.data)]] as [string, string | undefined][]).map(([lbl, val]) => (
                <div key={lbl} style={{ marginBottom: 12 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', color: COLORS.grayText, marginBottom: 4 }}>{lbl}</div>
                  <div style={{ fontSize: 13, color: COLORS.text }}>{val ?? '—'}</div>
                </div>
              ))}
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', color: COLORS.grayText, marginBottom: 4 }}>Canal</div>
                <CanalPill canal={detailLead.origem} />
              </div>
            </div>
            ...
            <a href={`https://wa.me/55${(detailLead.whatsapp ?? '').replace(/\D/g, '')}`} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: 9, background: '#25D366', color: '#fff', borderRadius: 8, fontSize: 13, fontWeight: 600, textDecoration: 'none', marginTop: 10 }}>
              💬 Abrir no WhatsApp
            </a>
          </div>
        )}
